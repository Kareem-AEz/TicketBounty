...
["todos"][("todos", { id: 1 })][("users", userId, "posts")]; // Simple key // With parameters // Hierarchical
...
async function getComments({
  ...
  pageParam?: { id: string; createdAt: number };
}) {
...
["user"][("users", userId)][("user-profile", userId)][
  // ✅ GOOD: Consistent hierarchy
  "users"
][("users", userId)][("users", userId, "posts")][ // All users // Specific user // User's posts
